...
# 旗帜配置
FLAG_CONFIG = {
    "red": {"value": 5, "name": "红方旗帜"},  # 红方旗帜在TERRAIN_MAP中值为5
    "blue": {"value": 4, "name": "蓝方旗帜"}  # 蓝方旗帜在TERRAIN_MAP中值为4
}
# 反向映射：地形值 → 所属阵营
TERRAIN_TO_FLAG_FACTION = {
    4: "blue",
    5: "red"
}

# -------------------------- 地形定义（9x9山峰分布） --------------------------
# 0=不可通行，1=平地 2=山峰 3=树林 4=蓝方旗帜 5-红方旗帜（可根据实际地图调整坐标）
TERRAIN_MAP = [
    [1, 1, 2, 0, 1, 0, 1, 3, 1],
    [2, 3, 1, 1, 4, 1, 1, 2, 2],
    [3, 1, 1, 0, 1, 0, 1, 1, 3],
...
# -------------------------- 兵种属性 --------------------------
TROOP_STATS = {
    "infantry":       {"atk": 2, "def": 7, "max_actions": 2, "max_attacks": 1},   # 步兵：最多2行动/1攻击
    "heavy_cavalry":  {"atk": 3, "def": 7, "max_actions": 3, "max_attacks": 1},   # 重骑兵：最多3行动/1攻击
    "light_cavalry":  {"atk": 3, "def": 5, "max_actions": 3, "max_attacks": 1},   # 轻骑兵：最多3行动/1攻击
    "longbow":        {"atk": 2, "def": 4, "max_actions": 2, "max_attacks": 1},   # 长弓手：最多2行动/1攻击
    "crossbow":       {"atk": 3, "def": 4, "max_actions": 2, "max_attacks": 1},   # 弩手：最多2行动/1攻击
}
...
class ChessPiece:
    ...
    def __init__(self, faction, troop_type, grid_x, grid_y):
        ...
        self.max_actions = stats["max_actions"]  # 每回合最大行动次数
        self.max_attacks = stats["max_attacks"]  # 每回合最大攻击次数
        self.remaining_actions = self.max_actions  # 剩余行动次数
        self.used_attacks = 0  # 已攻击次数
        
        # 暴露状态细分（区分"攻击后暴露"和"被发现暴露"）
    ...
    def draw_attributes(self, surface, mouse_pos):
        ...
        text_actions = FONT_ATTR.render(f"行动：{self.remaining_actions}/{self.max_actions}", True, (0, 0, 220))
        text_attacks = FONT_ATTR.render(f"攻击：{self.used_attacks}/{self.max_attacks}", True, (128, 0, 128))
        
        # 绘制文本
        surface.blit(text_name, (box_x + 5, box_y + 5))
        surface.blit(text_atk, (box_x + 5, box_y + 25))
        surface.blit(text_def, (box_x + 5, box_y + 45))
        surface.blit(text_actions, (box_x + 5, box_y + 65))
        surface.blit(text_attacks, (box_x + 5, box_y + 85))
    ...
    def reset_action_counts(self):
        ...
        self.used_attacks = 0
        self.attack_exposed = False
        self.discovered_exposed = False
...
def handle_attack(attacker, defender, pieces):
    ...
    # 规则1：重骑兵攻击时，被攻击方不反击
    if attacker.troop_type == "heavy_cavalry" and not defender.is_on_mountain():
...
def check_flag_hold(pieces, flag_hold_count):
    """
    检查夺旗状态：仅占领敌方旗帜且累计停留3回合才计数获胜
    :param pieces: 所有棋子列表
    :param flag_hold_count: 夺旗计数字典 {阵营: {棋子: 停留回合数}}
    :return: 夺旗结果 (阵营/None, 夺旗方式/None)
    """
    # 1. 先清理：移除已死亡/离开敌方旗帜的棋子计数
    for faction in ["red", "blue"]:
        enemy_flag_value = FLAG_CONFIG["blue" if faction == "red" else "red"]["value"]
        # 遍历当前阵营的计数棋子，移除无效项
        for piece in list(flag_hold_count[faction].keys()):
            # 条件：棋子已死亡 或 不在敌方旗帜位置
            if (piece not in pieces or 
                TERRAIN_MAP[piece.grid_y][piece.grid_x] != enemy_flag_value):
                flag_hold_count[faction].pop(piece)
    
    # 2. 判定是否有棋子满足3回合占领
    red_winner = None
    blue_winner = None
    # 检查红方占领蓝方旗帜（蓝方旗帜值=4）
    for piece in flag_hold_count["red"]:
        if flag_hold_count["red"][piece] >= 3:
            red_winner = piece
    # 检查蓝方占领红方旗帜（红方旗帜值=5）
    for piece in flag_hold_count["blue"]:
        if flag_hold_count["blue"][piece] >= 3:
            blue_winner = piece
    ...
    if red_winner and blue_winner:
        ...
        return ("red", "夺旗（占领蓝方旗帜）")
    elif blue_winner:
        return ("blue", "夺旗（占领红方旗帜）")
    return (None, None)

def check_total_elimination(pieces):
    """
    检查全剿状态：判断是否一方棋子被全部消灭
    :return: 全剿结果 (阵营/None, 全剿方式/None)
    """
    red_pieces = [p for p in pieces if p.faction == "red"]
    blue_pieces = [p for p in pieces if p.faction == "blue"]
    
    if not red_pieces and not blue_pieces:
        return ("draw", "平局")  # 双方同时被全剿
    elif not red_pieces:
        return ("blue", "全剿")
...
def run_game_play():
    ...
    # ====================== 胜负判定新增变量 ======================
    # 夺旗计数：{阵营: {棋子: 停留回合数}}
    flag_hold_count = {"red": {}, "blue": {}}
    game_result = None  # 游戏结果：None/("red", "夺旗")/("blue", "全剿")/("draw", "平局")
    ...
    selected_piece = None
    # 在 run_game_play 函数中，初始化 flag_hold_count 后添加：
    flag_hold_count = {"red": {}, "blue": {}}
    # 清空开局在己方旗帜上的棋子计数
    for piece in pieces:
        terrain_val = TERRAIN_MAP[piece.grid_y][piece.grid_x]
        if terrain_val in TERRAIN_TO_FLAG_FACTION:
            flag_owner = TERRAIN_TO_FLAG_FACTION[terrain_val]
            if piece.faction == flag_owner:  # 棋子在己方旗帜上
                if piece in flag_hold_count[piece.faction]:
                    flag_hold_count[piece.faction].pop(piece)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                ...
                # 重置刚结束回合方的棋子行动次数+暴露状态
                ended_turn = TURN_BLUE if current_turn == TURN_RED else TURN_RED
                reset_count = 0
                for piece in pieces:
                    if piece.faction == ended_turn:
                        piece.reset_action_counts()
                        reset_count += 1
                print(f"🔄 {ended_turn}方{reset_count}枚棋子行动次数已重置")
                ...
                if current_turn == TURN_RED:
                    ...
                    for piece in pieces:
                        terrain_val = TERRAIN_MAP[piece.grid_y][piece.grid_x]
                        # 仅处理旗帜地形（4=蓝旗，5=红旗）
                        if terrain_val not in TERRAIN_TO_FLAG_FACTION:
                            continue
                        flag_owner = TERRAIN_TO_FLAG_FACTION[terrain_val]
                        piece_faction = piece.faction
                        # 仅对「占领敌方旗帜」的棋子累加计数
                        if piece_faction != flag_owner:
                            if piece in flag_hold_count[piece_faction]:
                                flag_hold_count[piece_faction][piece] += 1
                                print(f"📌 {piece_faction}方{piece.troop_type}占领敌方旗帜，累计{flag_hold_count[piece_faction][piece]}回合")
                            else:
                                flag_hold_count[piece_faction][piece] = 1
                                print(f"📌 {piece_faction}方{piece.troop_type}首次占领敌方旗帜，累计1回合")
                ...
                ended_turn = TURN_BLUE if current_turn == TURN_RED else TURN_RED  # 刚结束的回合阵营
                reset_count = 0
                for piece in pieces:
                ...
                if not game_over:
                    # 检测夺旗胜利
                    flag_winner, flag_type = check_flag_hold(pieces, flag_hold_count)
                    # 检测全剿胜利
                    elim_winner, elim_type = check_total_elimination(pieces)
                    
                    # 优先判定夺旗，其次全剿
                    if flag_winner:
                        game_result = (flag_winner, flag_type)
                        game_over = True
                        print(f"🏆 游戏结束！{flag_winner}方通过{flag_type}获胜！")
                    elif elim_winner:
                        game_result = (elim_winner, elim_type)
                        game_over = True
                        print(f"🏆 游戏结束！{elim_winner}方通过{elim_type}获胜！")
                ...
                if clicked_piece:
                    ...
                    if selected_piece and selected_piece != clicked_piece:
                        if selected_piece.faction != clicked_piece.faction:
                            # 攻击前校验：行动次数、攻击次数
                            if selected_piece.remaining_actions <= 0:
                                print(f"❌ {selected_piece.troop_type}行动次数已耗尽（剩余{selected_piece.remaining_actions}/{selected_piece.max_actions}），无法攻击！")
                                continue
                            if selected_piece.used_attacks >= selected_piece.max_attacks:
                                print(f"❌ {selected_piece.troop_type}本回合攻击次数已耗尽（{selected_piece.used_attacks}/{selected_piece.max_attacks}），无法攻击！")
                                continue
                            # 判断攻击范围是否合法
                            ...
                            if distance <= attack_range:
                                ...
                                if selected_piece.faction == current_turn:
                                    ...
                                    selected_piece.remaining_actions -= 1
                                    selected_piece.used_attacks += 1
                                    # 攻击后取消选中
                                    selected_piece.is_selected = False
                                    selected_piece = None
                                else:
            ...
            if event.type == pygame.KEYDOWN and selected_piece:
                ...
                # 移动前校验
                ...
                elif selected_piece.remaining_actions <= 0:
                    print(f"❌ {selected_piece.troop_type}行动次数已耗尽（剩余{selected_piece.remaining_actions}/{selected_piece.max_actions}），无法移动！")
                    ...
                    if game_map.is_valid_position(new_x, new_y):
                        if TERRAIN_MAP[new_y][new_x] == 0:
                            print(f"❌ 移动失败：({new_x},{new_y}) 位置不可通行")
                            continue  # 终止本次移动逻辑
                        # 检查目标位置是否有敌方隐藏棋子（树林中）
                        hidden_enemy = None
                        for piece in pieces:
                        ...
                        if hidden_enemy:
                            # 移动到有隐藏敌方棋子的树林 → 触发攻击（需额外校验攻击次数）
                            if selected_piece.used_attacks >= selected_piece.max_attacks:
                                print(f"❌ {selected_piece.troop_type}本回合攻击次数已耗尽，无法触发强制攻击！")
                                continue
                            # 移动到有隐藏敌方棋子的树林 → 触发攻击
            ...
            # 游戏结束后禁用所有操作（保留ESC返回主页）
            if game_over:
                if event.type not in [pygame.QUIT, pygame.KEYDOWN] or (event.type == pygame.KEYDOWN and event.key != pygame.K_ESCAPE):
                    continue
        ...
        # ====================== 绘制胜负结果 ======================
        if game_over:
            # 绘制结果文本（右上角位置，可自行调整坐标）
            if game_result[0] == "draw":
                result_text = FONT_TITLE.render("双方平局！", True, (128, 0, 128))
            else:
                faction_name = "红方" if game_result[0] == "red" else "蓝方"
                result_text = FONT_TITLE.render(f"{faction_name}{game_result[1]}成功！", True, (255, 0, 0) if game_result[0] == "red" else (0, 0, 255))
            
            # 文本位置：右上角（窗口宽度-文本宽度-20，20）
            result_rect = result_text.get_rect(topright=(config.WINDOW_WIDTH - 20, 20))
            # 绘制文本背景框（增强可读性）
            bg_rect = pygame.Rect(result_rect.x - 10, result_rect.y - 5, result_rect.width + 20, result_rect.height + 10)
            pygame.draw.rect(game_core.screen, (255, 255, 255), bg_rect)  # 白色背景
            pygame.draw.rect(game_core.screen, (0, 0, 0), bg_rect, 3)     # 黑色边框
            game_core.screen.blit(result_text, result_rect)
